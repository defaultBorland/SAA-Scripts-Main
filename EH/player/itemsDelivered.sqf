...
"deliveredItems" addPublicVariableEventHandler {
	...
	if !(_deliveredItemsArray isEqualTo []) then {
		...
		player setVariable ["itemsDelivered_actionID", // addAction will return actionId which will be the value
			player addAction [localize "SAA_DELIVEREDITEMSLIST_ACTION", {
				(_this # 3) params ["_deliveredItemsArray"];

				createDialog "dialog_ListNBox";
				_display = (findDisplay 1000);
				(_display displayCtrl 1002) ctrlSetText localize "SAA_DELIVEREDITEMSLIST_HEADER"; // Set header text
				...
				_buttonHelp ctrlAddEventHandler ["ButtonDown",	{	
					...
					createDialog "Helper";
					((findDisplay 1100) displayCtrl 1110) ctrlSetText (localize "SAA_DELIVEREDITEMSLIST_HELP_1" + endl + endl + localize "SAA_DELIVEREDITEMSLIST_HELP_2" + endl + endl + localize "SAA_DELIVEREDITEMSLIST_HELP_3");
