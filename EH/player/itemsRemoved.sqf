...
"removedItems" addPublicVariableEventHandler {
	...
	if !(_removedItemsArray isEqualTo []) then {
		...
		player setVariable ["itemsRemoved_actionID", // addAction will return actionId which will be the value
			player addAction [localize "STR_SAA_REMOVEDITEMSLIST_ACTION", {
				(_this # 3) params ["_removedItemsArray"];

				createDialog "dialog_ListNBox";
				_display = (findDisplay 1000);
				(_display displayCtrl 1002) ctrlSetText localize "STR_SAA_REMOVEDITEMSLIST_HEADER"; // Set header text
				...
				_buttonHelp ctrlAddEventHandler ["ButtonDown",	{
					...
					createDialog "Helper";
					((findDisplay 1100) displayCtrl 1110) ctrlSetText (localize "STR_SAA_REMOVEDITEMSLIST_HELP_1" + endl + endl + localize "STR_SAA_REMOVEDITEMSLIST_HELP_2" + endl + endl + localize "STR_SAA_REMOVEDITEMSLIST_HELP_3");
