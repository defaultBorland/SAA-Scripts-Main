...
if (_unit getVariable ["SAA_isZeus", false]) then {
	[
		[
			...
			[format["%1 | %2 | %3:%4", toUpper worldName, mapGridPosition _unit, date select 3, date select 4], "<t align = 'left' shadow = '1' size = '0.7'>%1</t><br/>", 40]
		], 1, 1
	...
	[
		[
			...
			[format["%1 | %2 | %3:%4", toUpper worldName, mapGridPosition _unit, date select 3, date select 4], "<t align = 'left' shadow = '1' size = '0.7'>%1</t><br/>", 8],
