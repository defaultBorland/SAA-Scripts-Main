...
switch (_type) do {
	case "OnlyTeamLeaders": {
		...
		_players = _players apply {[format ["%1 (%2)", name _x, count units group _x]]}; // Format to Name (group count)
	};
	case "OnlyWithoutSquad": {
		...
		_players = _players - [_player]; // Remove caller
		_players = _players apply {[format ["%1 (%2)", name _x, count units group _x]]}; // Format to Name 
	};
	case "OnlySquadMembers": {
		_players = _players select {(leader group _x) isEqualTo _player};
		_players = _players - [_player];
		_players = _players apply {[format ["%1", name _x]]}; // Format to Name 
	};
	case "All";
	default {
		_players = _players - [_player];
