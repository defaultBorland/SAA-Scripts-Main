...
_insertChildren = {
	...
	_players = ["OnlyWithoutSquad", _player] call Shadec_fnc_getPlayersByGroupState;
    ...
    private _actions = [];
    {
        private _childStatement = {
			...
			[{
				[["All", side (_this # 0), true], Shadec_fnc_playersList] remoteExec ["call", 2];
...
//
_action = ["forceJoinToSquad", localize "STR_SAA_FORCE_JOIN_TO_SQUAD_ACTION_NAME", "img\Actions\joinToSquad.paa",
	{
		params ["_target", "_player", "_actionParams"];
		[["All", side _player], Shadec_fnc_playersList] remoteExec ["call", 2];
	},	// Statement
	{
		(player isEqualTo (leader group player)) 
		&& {(player getVariable ["SAA_Rank", "PV1"]) in ["CPT","1LT","2LT","CWO","WO1","SMC","MSG","SSG","SGT","SPC"]}
		&& {count (["OnlyWithoutSquad", _player] call Shadec_fnc_getPlayersByGroupState) > 0}
	},	// Condition
	_insertChildren,
	[],
	"",
