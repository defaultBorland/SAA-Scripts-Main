...
_insertChildren = {
    ...
    private _actions = [];
    {
        private _childStatement = {
			...
			[[_slave, _commander], {systemChat format ["> Server: %1 %2 %3.", name (_this # 0), localize "STR_SAA_FORCE_REMOVE_FROM_SQUAD_CHATINFO", name (_this # 1)]}] remoteExec ["call", -2];
        ...
        private _action = [format ["removePlayer:%1", _x # 1], _x # 1, "", _childStatement, {true}, {}, [_player, _x # 0]] call ace_interact_menu_fnc_createAction;
...
_action = ["forceRemoveFromSquad", localize "STR_SAA_FORCE_REMOVE_FROM_SQUAD_ACTION_NAME", "img\removeFromSquad.paa",
	...
	{(player isEqualTo (leader group player)) and {(player getVariable ["SAA_Rank", "PV1"]) in ["CPT","1LT","2LT","CWO","WO1","SMC","MSG","SSG","SGT","SPC"] and {(count units group player) > 1}}},	// Condition
	_insertChildren,
	[],
	"",
	4,
